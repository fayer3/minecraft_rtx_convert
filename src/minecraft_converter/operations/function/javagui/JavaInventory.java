...
public class JavaInventory {
    ...
    public static String compute(FileAccess access, String source, Settings config) throws ExitNow {
        try {
            ...
            current = new BufferedImage(pixelSize * 18, pixelSize * 15, BufferedImage.TYPE_INT_ARGB);
            for (int x = 0; x < current.getWidth(); x++) {
                for (int y = 0; y < current.getHeight(); y++) {
                    current.setRGB(x, y, icons.getRGB(x + pixelSize * 134, y + pixelSize * 28));
                }
            }
            outData = new ByteArrayOutputStream();
            ImageIO.write(current, "png", outData);
            ...
            access.zipAddFileStream("ui/inventory_screen.json", JavaHorse.class.getClassLoader().getResourceAsStream("bedrock/inventory_screen.json"));
            files += ";ui/inventory_screen.json";
